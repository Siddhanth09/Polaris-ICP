...
actor {
  ...
  type leaves = Text;
  type MerkelRoot = Text;
  type rootindex = Text;

  type Entry = {
    merkelRoot: MerkelRoot;
  };
  let MerkelRecord = Map.HashMap<rootindex, Entry>(0, Text.equal, Text.hash);

  public func insertRoot(rootindex: rootindex, entry : Entry): async () {
    MerkelRecord.put(rootindex, entry);
  };

  public query func lookupCommit(index :rootindex ) : async ?Entry {
    MerkelRecord.get(index);
  };
...
// ... (Previous code remains unchanged)
  ...
  public func Verify_tree(proof: Text, leaf: Text, root: Text) : async Text {
    ...
    let request_body_json: Text = "{\"proof\": \"" # proof # "\", \"leaf\": \"" # leaf # "\", \"root\": \"" # root # "\"}";
